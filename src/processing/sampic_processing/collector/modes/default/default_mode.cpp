...
SAMPIC_REGISTER_MODE(
    ...
    FrontendCollectorModeDefaultConfig,
    "default",
    "Hit-time clustering",
    [](const FrontendCollectorModeDefaultConfig& config) {
...
bool FrontendCollectorModeDefault::collect()
{
    ...
    // groups retain shared ownership of every parent they still reference.
    ...
    return emitReadyGroups(t_start, wait_us, group_build_us);
}

bool FrontendCollectorModeDefault::flush() {
    while (!pending_groups_.empty()) {
        ready_groups_.emplace_back(std::move(pending_groups_.front()));
        pending_groups_.pop_front();
    }

    return emitReadyGroups(
        std::chrono::steady_clock::now(),
        std::chrono::microseconds{0},
        std::chrono::microseconds{0});
}

bool FrontendCollectorModeDefault::emitReadyGroups(
    std::chrono::steady_clock::time_point t_start,
    std::chrono::microseconds wait_us,
    std::chrono::microseconds group_build_us) {
    if (ready_groups_.empty()) {
        return true;
    }
    ...
    for (auto& g : groups_to_process) {
        ...
        if (diagnostics_config_.log_group_details && spdlog::should_log(spdlog::level::debug)) {
        ...
        if (diagnostics_config_.log_hit_details && spdlog::should_log(spdlog::level::debug)) {
            size_t idx = 0;
