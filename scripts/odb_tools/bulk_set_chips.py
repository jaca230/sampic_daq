...
"""
Bulk update chip-level parameters in the SAMPIC frontend ODB tree.

Example usage:

    ./bulk_set_chips.py baseline_reference 0.45
    ./bulk_set_chips.py enable_post_trigger true --boards 0,1
"""
...
def build_paths(frontend: str,
    ...
    for board in boards:
        for chip in chips:
            yield f"{base}/feb{board}/sampics/sampic{chip}/{field}"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Bulk update SAMPIC chip-level ODB parameters."
    )
    parser.add_argument(
        "field",
        help="Chip field name to update (e.g. baseline_reference, enable_post_trigger).",
    )
    parser.add_argument(
        "value",
    ...
    paths = list(build_paths(frontend, boards, chips, args.field))

    if not paths:
        print("Nothing to update.")
        return 0

    client = midas.client.MidasClient("sampic_chip_bulk_update")
