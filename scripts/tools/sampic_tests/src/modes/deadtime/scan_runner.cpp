...
nlohmann::json build_record(const DeadtimeConfig& cfg,
  ...
  record["parameters"] = {
      {"pulser_period_us", combo.pulser_period_us},
      {"digitizer_rate_mhz", combo.digitizer_rate_mhz},
      {"pulser_period_ticks", combo.pulser_period_us},
      {"channel_count", combo.channel_set.channels.size()},
      {"channel_ids", combo.channel_set.channels},
      {"channel_label", combo.channel_set.label},
  ...
  if (agg.total_duration_s > 0.0) {
    ...
    record["aggregate"]["events_per_second"] = nlohmann::json();
    record["aggregate"]["data_mb_per_s"] = nlohmann::json();
  }
  if (agg.events > 0) {
    record["aggregate"]["hits_per_event"] =
        static_cast<double>(agg.total_hits) / static_cast<double>(agg.events);
  } else {
    record["aggregate"]["hits_per_event"] = nlohmann::json();
  }
  ...
  if (readback.has_value()) {
    record["readback"] = {
        ...
        {"pulser_period_ticks", readback->pulser_period_ticks},
...
void append_record(const std::filesystem::path& path, const nlohmann::json& record) {
  ...
  if (!out) {
    throw std::runtime_error("Unable to open results file for append: " + path.string());
  }
  out << record.dump() << '\n';
  out.flush();
}

std::unordered_set<std::string> load_completed_combos(const std::filesystem::path& path) {
  ...
  if (!in) {
    throw std::runtime_error("Unable to read existing results file: " + path.string());
  }
  std::string line;
  while (std::getline(in, line)) {
    if (line.empty()) continue;
    const nlohmann::json record = nlohmann::json::parse(line, nullptr, true, true);
    if (!record.contains("status")) continue;
    const std::string status = record.at("status").get<std::string>();
    if (status != "complete") continue;
    const std::string key = make_combo_key_from_json(record);
    if (!key.empty()) {
...
DeadtimeScanRunner::DeadtimeScanRunner(DeadtimeConfig config,
                                       volatile std::sig_atomic_t* stop_flag)
    : config_(std::move(config)), stop_flag_(stop_flag) {}

int DeadtimeScanRunner::run(bool debug_mode) {
  auto combos = build_parameter_space(config_.scan);
  auto completed = load_completed_combos(config_.output.results_path);
  ...
  auto make_session = [&]() {
    return std::make_unique<SampicSession>(config_.connection);
  };
  ...
  for (const auto& combo : combos) {
    ...
    std::cout << "[run ] (" << combo_index << "/" << combos.size()
              << ") pulser=" << combo.pulser_period_us << "us"
              << " digitizer=" << combo.digitizer_rate_mhz << "MHz"
              << " channels=" << combo.channel_set.channels.size()
              << " label=" << combo.channel_set.label << std::endl;
    ...
    for (int attempt = 0; attempt < max_session_attempts && !active_session; ++attempt) {
      try {
        ...
        sess.configure_for_combo(combo, config_.scan.board_index);
        active_session = &sess;
      } catch (const std::exception& ex) {
        ...
        if (attempt + 1 >= max_session_attempts) {
          ...
          std::cerr << "Configuration failed (" << config_error
                    << "). Reconnecting to crate and retrying...\n";
        }
        session.reset();
      }
    }
    ...
    if (active_session && !combo_failed) {
      ...
      if (!active_session->start_run_with_retry(config_.start_retry, start_attempts, start_errors)) {
        ...
        if (debug_mode && !start_errors.empty()) {
          for (const auto& err : start_errors) {
            std::cout << "    start error: " << err << "\n";
          }
        }
        SampleResult failure;
        ...
        samples.push_back(failure);
        report_sample(samples.back(), -1, config_.timing.samples_per_combo);
        std::cerr << "StartRun failed after " << start_attempts << " attempts.\n";
        session.reset();
      } else {
    ...
    const auto record =
        build_record(config_, combo, samples, readback_settings, readback_error,
                     run_status, status, key, wall_seconds);
    append_record(config_.output.results_path, record);
    if (status == "complete") {
      completed.insert(key);
    }
    std::cout << "  → status: " << status << ", duration=" << std::fixed
              << std::setprecision(1) << wall_seconds << "s" << std::endl;
  }
